    ...
    <div className="max-w-4xl mx-auto px-4 py-12 sm:py-24 text-center space-y-20">
      ...
      <div className="grid sm:grid-cols-2 gap-6 max-w-3xl mx-auto">
        ...
        >
          ...
          <div className="space-y-2">
            <h3 className="text-2xl sm:text-3xl font-black italic uppercase tracking-tighter">Սկսել Խաղը</h3>
...
function RulesView({ onBack }: { onBack: () => void }) {
  return (
    <div className="max-w-4xl mx-auto px-4 py-12 sm:py-24 space-y-12">
      <button onClick={onBack} className="flex items-center gap-2 text-slate-400 hover:text-indigo-600 transition-colors font-black uppercase text-xs tracking-widest">
         <ArrowLeft className="w-4 h-4" /> ԵՏ
      </button>
      <div className="space-y-12">
        <h2 className="text-3xl sm:text-7xl font-black uppercase italic tracking-tighter">Pretérito <span className="text-indigo-600">Perfecto</span></h2>
        ...
        <div className="bg-indigo-600 p-8 sm:p-12 rounded-[3.5rem] text-white space-y-4">
           <div className="flex items-center gap-3">
             <Sparkles className="w-6 h-6" />
             <h3 className="text-2xl font-black uppercase italic">Ե՞րբ օգտագործել</h3>
           </div>
           <p className="font-bold italic opacity-90 leading-relaxed text-lg italic">
             Օգտագործվում է անցյալում կատարված այն գործողությունների համար, որոնք կապված են ներկայի հետ: 
             Հաճախ օգտագործվում է հետևյալ բառերի հետ՝ <span className="underline decoration-indigo-300">hoy</span> (այսօր), 
             <span className="underline decoration-indigo-300">ya</span> (արդեն), 
             <span className="underline decoration-indigo-300">esta semana</span> (այս շաբաթ), 
             <span className="underline decoration-indigo-300">alguna vez</span> (երբևէ):
           </p>
        </div>
      </div>
    </div>
...
function RuleCard({ title, items, icon }: { title: string, items: {es: string, hy: string}[], icon: any }) {
  return (
    <div className="bg-white p-6 sm:p-8 rounded-[2.5rem] shadow-xl border border-slate-50 space-y-6">
      ...
      <div className="space-y-4">
        {items.map(it => (
          <div key={it.es}>
             <span className="text-indigo-600 font-black text-lg sm:text-xl italic">{it.es}:</span>
             <p className="text-slate-500 font-bold text-sm sm:text-base">{it.hy}</p>
          </div>
        ))}
      </div>
    </div>
...
function ArrowLeft(props: any) {
  return <svg {...props} xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round"><path d="m12 19-7-7 7-7"/><path d="M19 12H5"/></svg>
}
  ...
  return (
    <div className="min-h-screen bg-slate-50 font-sans text-slate-900 selection:bg-indigo-100 selection:text-indigo-900 overflow-x-hidden">
      <AnimatePresence mode="wait">
        ...
        {screen === 'rules' && (
          <motion.div key="rules" initial={{ opacity: 0, x: 50 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -50 }}>
